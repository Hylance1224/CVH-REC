...
class Model_Wrapper(object):
    def __init__(self, data_config, pretrain_data):
        ...
        (self.norm_mashup_call_1, self.norm_mashup_call_2, self.norm_api_call_1, self.norm_api_call_2,
         self.norm_mashup_tag_1, self.norm_mashup_tag_2, self.norm_api_tag_1,
         self.norm_api_tag_2) = self.build_hyper_edge(
            args.data_path + args.dataset + '/TE.csv', args.data_path + args.dataset + '/mashup_tags.csv',
            args.data_path + args.dataset + '/api_tags.csv')
        print(
            f"norm_mashup_call_1 shape: {self.norm_mashup_call_1.shape}, norm_mashup_call_2 shape: {self.norm_mashup_call_2.shape}")
    ...
    def build_hyper_edge(self, mashup_api_call_file, mashup_tag_call_file, api_tag_call_file):
        ...
        # mashup调用API交互矩阵
        mashup_api_call_matrix = read_and_fill_matrix(mashup_api_call_file, (self.n_mashup, self.n_api),
                                                      transpose=False)
        # API调用mashup矩阵（转置）
        ...
        mashup_tag_call_matrix = read_and_fill_matrix(mashup_tag_call_file, (self.n_mashup, self.n_tag),
                                                      transpose=False)
        ...
        # 构造mashup超边
        norm_mashup_call_1, norm_mashup_call_2 = build_and_normalize_hyperedge(mashup_api_call_matrix, alpha1,
                                                                               self.n_mashup,
                                                                               'mashup')
        # 构造api超边
