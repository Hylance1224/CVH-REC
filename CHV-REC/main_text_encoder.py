...
import sys
import math
from Models_text_encoder import UserTextEncoder
...
if args.method == "CVH-REC":
    from Models import *
elif args.method == "Random_init":
    from Models_random_init import *
elif args.method == "Without_local_CL":
    from Models_without_local_CL import *
elif args.method == "Without_global_CL":
    from Models_without_global_CL import *
elif args.method == "Without_CL":
    from Models_without_CL import *
elif args.method == "Without_tag_view":
    from Models_without_single_view import *
elif args.method == "Without_call_view":
    from Models_without_single_view import *
elif args.method == "Local":
    from Models_local import *
elif args.method == "Global":
    from Models_global import *
elif args.method == "Without_multi":
...
class Model_Wrapper(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.weights_save_path = (
            f"{args.weights_path}weights/{self.method}/{args.dataset}/{args.seed}/"
            f"alpha-{alpha_str}/"
        ...
        self.DNN_weights_save_path = (
            f"{args.weights_path}DNN_weights/{self.method}/{args.dataset}/{args.seed}/model"
        )
    ...
    def save_model(self):
        ensureDir(self.DNN_weights_save_path)
        torch.save(self.text_encoder.state_dict(), self.DNN_weights_save_path)

    def load_model(self):
        self.model.load_state_dict(torch.load(self.weights_save_path))

    def load_DNN_model(self):
        self.text_encoder.load_state_dict(torch.load(self.DNN_weights_save_path))
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            if self.method == "Global":
                (m_embeddings, a_embeddings, mashup_tag_logits, api_tag_logits) = (
                    self.model(self.global_1, self.global_2))

            if self.method == "Without_CL":
                (m_embeddings, a_embeddings,
                 mashup_tag_logits, api_tag_logits) = (
                    self.model(self.norm_mashup_call_1, self.norm_mashup_call_2,
                               self.norm_api_call_1, self.norm_api_call_2,
                ...
                (m_embeddings, a_embeddings,
                 ...
                 mashup_tag_logits, api_tag_logits) = (
                    self.model(self.norm_mashup_call_1, self.norm_mashup_call_2,
                               ...
                               self.norm_mashup_tag_1, self.norm_mashup_tag_2,
                               self.norm_api_tag_1, self.norm_api_tag_2, self.global_1, self.global_2))
            if self.method == "Without_global_CL":
                (m_embeddings, a_embeddings,
                 m_c_proj, m_t_proj, a_c_proj, a_t_proj,
                 mashup_tag_logits, api_tag_logits) = (
                    self.model(self.norm_mashup_call_1, self.norm_mashup_call_2,
                               ...
                               self.norm_mashup_tag_1, self.norm_mashup_tag_2,
                               self.norm_api_tag_1, self.norm_api_tag_2, self.global_1, self.global_2))
            if self.method == "Without_local_CL":
                (m_embeddings, a_embeddings,
                 m_local_proj, a_local_proj, m_global_proj, a_global_proj,
                 mashup_tag_logits, api_tag_logits) = (
                    self.model(self.norm_mashup_call_1, self.norm_mashup_call_2,
                               ...
                               self.norm_mashup_tag_1, self.norm_mashup_tag_2,
                               self.norm_api_tag_1, self.norm_api_tag_2, self.global_1, self.global_2))
            if self.method == "Without_tag_view":
                (m_embeddings, a_embeddings,
                 m_local_proj, a_local_proj, m_global_proj, a_global_proj,
                 mashup_tag_logits, api_tag_logits) = (
                    self.model(self.norm_mashup_call_1, self.norm_mashup_call_2,
                               self.norm_api_call_1, self.norm_api_call_2, self.global_1, self.global_2))
            if self.method == "Without_call_view":
                (m_embeddings, a_embeddings,
                 m_local_proj, a_local_proj, m_global_proj, a_global_proj,
                 mashup_tag_logits, api_tag_logits) = (
                    self.model(self.norm_mashup_tag_1, self.norm_mashup_tag_2,
                               self.norm_api_tag_1, self.norm_api_tag_2, self.global_1, self.global_2))
        ...
        if args.save_flag == 1:
            self.save_model()
            print('save the weights in path: ', self.DNN_weights_save_path)
    ...
    def validation(self, a_embeddings):
        import numpy as np
        import torch

        va_file = f'dataset/{args.dataset}/VA.csv'
        ground_truth = {}
        with open(va_file, 'r', encoding='utf-8') as f:
            for line in f:
                parts = line.strip().split()
                mashup_id = int(parts[0])
                api_ids = list(map(int, parts[1:]))
                ground_truth[mashup_id] = set(api_ids)

        users_to_test = list(ground_truth.keys())

        # === 🧠 批量取出 mashup embedding ===
        user_vectors = []
        valid_users = []
        for user in users_to_test:
            user_vectors.append(self.user_text_embeddings[user])
            valid_users.append(user)
            # print(user)
            # print(self.user_text_embeddings[user])
            # print('=========')
        user_vectors = torch.stack(user_vectors, dim=0).to(a_embeddings.device)

        # === 🔥 一次性计算所有 mashup 的增强向量 ===
        enhanced_embeddings = self.text_encoder(user_vectors)

        # === 🚀 批量矩阵乘法（N_mashup × N_API） ===
        # rate_matrix[i, j] = score of mashup_i on API_j
        rate_matrix = torch.matmul(enhanced_embeddings, a_embeddings.T)

        # === ⚡ topk 批量选取 ===
        top_k = 20
        topk_indices = torch.topk(rate_matrix, top_k, dim=1).indices.cpu().numpy()

        # === 📊 计算整体 Recall@5 ===
        def recall_at_k(recommended, true_items, k=5):
            return len(set(recommended[:k]) & true_items) / len(true_items) if true_items else 0.0

        total_recall = 0.0
        for i, user in enumerate(valid_users):
            rec = topk_indices[i]
            recall5 = recall_at_k(rec, ground_truth[user], k=5)
            total_recall += recall5

        avg_recall5 = total_recall / len(valid_users)
        print(f"📊 Validation Recall@5: {avg_recall5:.4f}")
        return avg_recall5
    ...
    def save_recResult(self):
            ...
            if self.method == "Without_call_view":
                (m_embeddings, a_embeddings,
                 ...
                 mashup_tag_logits, api_tag_logits) = (
                    self.model(self.norm_mashup_tag_1, self.norm_mashup_tag_2,
                               self.norm_api_tag_1, self.norm_api_tag_2, self.global_1, self.global_2))
        # a_embeddings = self.load_all_item_text_embeddings(n_users=940, h5_path='data/API_vectors.h5').cuda()
        ...
        output_file = f'{args.output}/{self.method}_{args.dataset}.json'
